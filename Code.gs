...
function generateQuote(data) {
  ...
  const qaFee = 85.00;
  const adminFee = 315.00;

  // Full Tech Roster Burden Mapping
  const techBurdens = {
    "Sam": 28.25,
    "Lucas": 23.25,
    "Caden": 31.50,
  ...
  // Suggested Price = (Breakeven + 10% Risk Buffer) / Target Net Margin
...
function submitJobQualitative(data) {
  ...
  // Saving qualitative decision flags to custom columns
